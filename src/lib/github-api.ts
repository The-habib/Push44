...
export const getGitHubUser = createServerFn({ method: "POST" }).handler(
  async (ctx) => {
...
export const listGitHubRepos = createServerFn({ method: "POST" }).handler(
  async (ctx) => {
    const { token } = ctx.data as { token: string };
...
export const createGitHubRepo = createServerFn({ method: "POST" }).handler(
  async (ctx) => {
    const { token, name, isPrivate } = ctx.data as {
      token: string;
      name: string;
      isPrivate: boolean;
    };
...
export const pushFilesToGitHub = createServerFn({ method: "POST" }).handler(
  async (ctx) => {
    const { token, owner, repo, branch, files, commitMessage } = ctx.data as {
      token: string;
      owner: string;
      repo: string;
      branch: string;
