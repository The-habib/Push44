      ...
      <AppShell>
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-5 text-center">
          ...
          <div>
            ...
            <p className="text-[13px] text-[#6b6360] max-w-[200px] leading-relaxed">Connect your Base44 and GitHub accounts in Settings first.</p>
          </div>
          <MotionButton
    ...
    <AppShell>
      ...
      <FadeUp>
        <div className="mb-5">
          ...
          <p className="text-[13px] text-[#9a8880] mt-0.5">Select an app, pick a repo, and ship it.</p>
        </div>
      </FadeUp>

      {/* Step 1 — Select app */}
      <FadeUp delay={0.06}>
        <SectionShell step={1} label="Select Base44 App" active={!step1done} done={step1done}>
          {loadingApps ? (
            <div className="flex items-center justify-center gap-2 py-8 text-[13px] text-[#9a8880]">
              <Loader2 className="h-4 w-4 animate-spin text-[#f97316]" />Loading apps…
            </div>
          ) : apps.length === 0 ? (
            <div className="text-center py-6">
              <p className="text-[13px] text-[#9a8880] mb-2">No apps found.</p>
              <button onClick={loadApps} className="text-[#f97316] font-bold text-[13px]">Try again →</button>
            </div>
          ) : (
                ...
                return (
                  <StaggerItem key={app.id}>
                    <motion.button
                      ...
                      animate={{
                        borderColor: active ? "#f97316" : "#f0ece4",
                        background: active ? "#fff4ed" : "#fafafa",
                      }}
                      whileHover={{ x: 2 }}
                      whileTap={{ scale: 0.98 }}
          ...
          <AnimatePresence>
            {loadingFiles && selectedApp && (
              ...
              >
                ...
                <span>{wakingSandbox ? "Waking sandbox — takes ~30s…" : "Fetching files from Base44…"}</span>
              </motion.div>
            )}
          </AnimatePresence>
        </SectionShell>
