...
import {
  ...
  Minus, Circle, UploadCloud, RefreshCw, Filter,
} from "lucide-react";
...
function StagingBrowser({
  ...
  const sorted = [...files].sort((a, b) => {
    const order: Record<DiffStatus, number> = { new: 0, modified: 1, unchanged: 2 };
        ...
        <div className="px-4 py-3 border-b border-[#f5f2ee]">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[13px] font-black text-[#1a1a1a]">{stagedCount} / {files.length} staged</span>
            </div>
...
function PushPage() {
  ...
  useEffect(() => {
    ...
    setApps([]); setSelectedApp(null); setFiles([]); setDiffMap(new Map()); setStagedPaths(new Set());
    loadApps();
  }, [platform, isLoaded]);
  ...
  const handleSelectApp = async (app: App) => {
    setSelectedApp(app); setFiles([]); setDiffMap(new Map()); setStagedPaths(new Set());
    ...
    try {
      ...
      if (snapshot && (newCount + modifiedCount) > 0) {
        setCommitMsg(`Update ${app.name}: ${newCount > 0 ? `${newCount} new` : ""}${newCount > 0 && modifiedCount > 0 ? ", " : ""}${modifiedCount > 0 ? `${modifiedCount} modified` : ""}`);
      }
    } catch (e: any) {
  ...
  const handlePush = async () => {
    ...
    try {
      ...
      addHistory({
        ...
        filesCount: stagedFiles.length, stagedCount: stagedFiles.length, newCount, modifiedCount,
        ...
        <div className="flex flex-col min-h-[72vh] pt-4 pb-2">
          ...
          <FadeUp delay={0.24}>
            <div className="space-y-2.5">
              ...
              <MotionButton
                onClick={() => { setStatus("idle"); setSelectedApp(null); setFiles([]); setDiffMap(new Map()); setStagedPaths(new Set()); setPushStats(null); loadApps(); }}
      ...
      <SectionShell step={1} label="Select app & review files" active={!step1done} done={step1done}>
        ...
        <AnimatePresence>
          {files.length > 0 && !loadingFiles && (
            <div className="mt-3">
              <StagingBrowser files={files} diffMap={diffMap} stagedPaths={stagedPaths} onChange={setStagedPaths} hasSnapshot={hasSnapshot} />
              {stagedPaths.size === 0 && (
      ...
      <SectionShell step={3} label="Commit message & push" active={step1done && step2done} done={status === "done"}>
        {!step2done ? (
          ...
          <>
            ...
            <MotionButton
              onClick={handlePush}
              disabled={status === "pushing" || stagedFiles.length === 0 || !commitMsg.trim()}
              className="w-full flex items-center justify-center gap-2.5 rounded-2xl py-3.5 font-bold text-[14px] text-white"
              style={{
                background: stagedFiles.length === 0 ? "#e5e0d8" : "linear-gradient(135deg,#f97316,#ea580c)",
                boxShadow: stagedFiles.length > 0 ? "0 4px 20px rgba(249,115,22,0.35)" : "none",
