...
import { FadeUp, StaggerContainer, StaggerItem, MotionCard, MotionButton, ScaleIn } from "@/components/PageTransition";
import { useState, useEffect } from "react";
import { motion } from "framer-motion";
import {
  ArrowRight, Clock, GitBranch, Loader2, AlertTriangle,
  Zap, LayoutGrid, CheckCircle2, XCircle,
} from "lucide-react";
...
function Stat({ label, value, loading, bg, accent, icon: Icon }: {
  label: string; value: number; loading?: boolean;
  bg: string; accent: string; icon: React.ElementType;
}) {
  return (
    <motion.div
    ...
    >
      <div className="h-10 w-10 rounded-2xl flex items-center justify-center" style={{ background: accent + "22" }}>
      ...
      <div>
        ...
        <div className="text-2xl font-extrabold text-black leading-none">
          {loading ? <Loader2 className="h-5 w-5 animate-spin text-black/30 mt-1" /> : (
            <motion.span
              key={value}
...
function Index() {
  ...
  useEffect(() => {
    ...
    listBase44Apps({ data: { token: creds.base44Token! } })
      .then(setApps).catch(() => {}).finally(() => setLoadingApps(false));
      ...
      <div className="relative z-10">
        ...
        <FadeUp delay={0.1}>
          ...
          >
            ...
            />
            <motion.div
              className="absolute -top-12 -right-12 h-56 w-56 rounded-full opacity-30 blur-3xl"
              style={{ background: "radial-gradient(circle,#7c3aed,transparent)" }}
              animate={{ scale: [1, 1.15, 1], opacity: [0.3, 0.5, 0.3] }}
              transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
            />
            <motion.div
              className="absolute -bottom-10 -left-10 h-44 w-44 rounded-full opacity-20 blur-3xl"
            ...
            <div className="relative z-10 px-6 pt-7 pb-7">
              <div className="inline-flex items-center gap-2 bg-white/10 border border-white/15 rounded-full px-3 py-1.5 mb-5">
                <motion.span
                  className={`h-1.5 w-1.5 rounded-full ${isConnected ? "bg-[#dce99a]" : "bg-white/30"}`}
                  animate={isConnected ? { scale: [1, 1.5, 1], opacity: [1, 0.6, 1] } : {}}
              ...
              <h2 className="text-[36px] leading-[1.02] font-extrabold tracking-tight mb-2">
                <span className="text-white">Push your</span><br />
              ...
              <MotionButton
                ...
                className="inline-flex items-center gap-2.5 rounded-2xl px-5 py-3.5 font-bold text-sm text-black shadow-lg"
        ...
        {!isConnected && (
          <FadeUp delay={0.15}>
            <MotionCard
              className="w-full flex items-center gap-3 rounded-2xl p-4 mb-4 text-left cursor-pointer"
              style={{ background: "linear-gradient(135deg,#fff7ed,#ffedd5)", border: "1px solid #fed7aa" } as any}
        ...
        <StaggerContainer className="grid grid-cols-3 gap-3 mb-4">
          <StaggerItem>
            <Stat label="Apps" value={apps.length} loading={loadingApps && isConnected} bg="#f0ebff" accent="#8b5cf6" icon={LayoutGrid} />
          </StaggerItem>
          <StaggerItem>
            <Stat label="Repos" value={repos.length} loading={loadingRepos && isConnected} bg="#f0fdf4" accent="#22c55e" icon={GitHubLogo} />
          </StaggerItem>
          <StaggerItem>
            <Stat label="Pushes" value={history.length} bg="#fff7ed" accent="#f97316" icon={Clock} />
          </StaggerItem>
        </StaggerContainer>
          ...
          <FadeUp delay={0.25}>
            <div className="bg-white rounded-3xl p-5 mb-4">
              ...
              <motion.div
                ...
                style={{ background: lastPush.status === "success" ? "linear-gradient(135deg,#f0fdf4,#dcfce7)" : "linear-gradient(135deg,#fef2f2,#fee2e2)" }}
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-right shrink-0">
                    <div className="text-[10px] font-bold px-2.5 py-1 rounded-full"
                      style={{ background: lastPush.status === "success" ? "#22c55e" : "#ef4444", color: "white" }}>
                ...
                <div className="flex items-center gap-3 mt-3 pt-3 border-t border-black/5">
                  <div className="text-[11px] text-black/50"><span className="font-semibold">{lastPush.filesCount}</span> files</div>
          ...
          <FadeUp delay={0.3}>
            <div className="bg-white rounded-3xl p-5 mb-4">
              ...
              {loadingRepos ? (
                ...
                <motion.div
                  className="flex items-center gap-3 bg-[#f9f8f4] rounded-2xl p-3"
                  whileHover={{ scale: 1.01 }}
                  transition={{ type: "spring", stiffness: 400, damping: 28 }}
                >
        ...
        {isConnected && (
          <FadeUp delay={0.35}>
            <motion.div
              ...
              whileHover={{ scale: 1.01, boxShadow: "0 12px 40px rgba(0,0,0,0.25)" }}
              transition={{ type: "spring", stiffness: 400, damping: 28 }}
            >
              <div>
              ...
              <MotionButton
                ...
                className="flex items-center gap-2 rounded-2xl px-4 py-3 font-bold text-sm text-black"
                style={{ background: "linear-gradient(135deg,#dce99a,#c5e352)" }}
              >
                <Zap className="h-4 w-4" strokeWidth={2.5} />
                Push
