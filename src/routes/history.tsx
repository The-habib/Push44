...
function HistoryPage() {
  ...
  const successRate = events.length === 0 ? 100 : Math.round((successCount / events.length) * 1000) / 10;
  const totalFiles = events.filter((e) => e.status === "success").reduce((sum, e) => sum + e.filesCount, 0);

  function handleClear() { clearHistory(); setEvents([]); }
      ...
      <div className="relative z-10">
        ...
        <FadeUp delay={0.05}>
          <section className="relative rounded-[32px] px-6 pt-7 pb-6 overflow-hidden mb-5" style={{ backgroundColor: "#fde2cf" }}>
            <motion.div
              ...
              style={{
                backgroundImage: "radial-gradient(circle at 80% 20%, #f97316 0%, transparent 60%)",
              }}
            ...
            <motion.div
              className="absolute right-4 bottom-4 h-2.5 w-2.5 rounded-full bg-[#8b5cf6]"
        ...
        <StaggerContainer className="grid grid-cols-2 gap-3 mb-5">
          <StaggerItem>
            ...
            >
              ...
              <div className="text-[11px] font-semibold text-black/40 uppercase tracking-wider mb-0.5">Total Pushes</div>
              <motion.div
                className="text-2xl font-extrabold text-black leading-none"
                key={events.length}
              ...
              {successCount > 0 && <div className="text-[10px] text-[#22c55e] font-semibold mt-1.5">{successCount} successful</div>}
            </motion.div>
          </StaggerItem>
          ...
          <StaggerItem>
            ...
            >
              ...
              <div className="text-[11px] font-semibold text-black/40 uppercase tracking-wider mb-0.5">Success Rate</div>
              <div className="text-2xl font-extrabold text-black leading-none">
                {events.length === 0 ? "—" : `${successRate}%`}
              </div>
              {failCount > 0 && <div className="text-[10px] text-[#ef4444] font-semibold mt-1.5">{failCount} {failCount === 1 ? "failure" : "failures"}</div>}
            </motion.div>
          </StaggerItem>
        </StaggerContainer>
        ...
        <FadeUp delay={0.2}>
          <SectionCard
            ...
            action={
              events.length > 0 ? (
                <MotionButton onClick={handleClear} className="flex items-center gap-1 text-[#ef4444] text-xs font-semibold">
          ...
          >
            {events.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 gap-3 text-center">
                <motion.div
                  className="h-12 w-12 rounded-2xl bg-[#f3f2ee] flex items-center justify-center"
                ...
                <button onClick={() => navigate({ to: "/push" })} className="text-sm font-bold text-[#8b5cf6]">Make your first push →</button>
              </div>
            ) : (
                  ...
                  {events.map((e, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <motion.div
                          className="flex-1 min-w-0 bg-[#fafaf7] rounded-xl p-3"
                          whileHover={{ backgroundColor: "#f5f4f0", x: 2 }}
                          transition={{ type: "spring", stiffness: 400, damping: 28 }}
                        >
                          <div className="flex items-center justify-between gap-2">
                            <div className="text-sm font-bold text-black truncate">{e.appName}</div>
                          ...
                          <div className="text-xs text-black/50 italic mt-0.5 truncate">"{e.commitMessage}"</div>
                          <div className="flex items-center gap-2 mt-2 flex-wrap">
                            ...
                            <span className="text-[10px] text-black/50">{e.filesCount} files</span>
                            {e.commitHash && (
                          ...
                          {!ok && e.error && (
                            <div className="mt-2 text-[11px] text-[#ef4444] bg-[#fee2e2] rounded-lg px-2 py-1 truncate">{e.error}</div>
                          )}
                        </motion.div>
                      </motion.li>
                    );
